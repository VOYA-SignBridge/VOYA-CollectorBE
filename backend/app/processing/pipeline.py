...
def process_video_job(video_path: str, user: str, label: str, session_id: str, dialect: str = "common", language: str = "vn"):
    ...
    try:
        ...
        aug_n = int(getattr(settings, "video_augment_per_seq", settings.augment_per_seq))
        ...
        # Live capture already targets the same cap; video uploads must not exceed it.
        max_per_class = int(settings.max_samples_per_class)
        existing_total = count_samples_for_class(class_meta.class_uid)
        remaining_quota = max(0, max_per_class - existing_total)
        if remaining_quota <= 0:
            logger.info(
                "[CAP] class=%s already has %s samples (max=%s); skipping video save.",
        ...
        if min_target > 0 and saved_total < min_target and best_windows:
            ...
            fill_n = min(need, remaining_quota - saved_total)
            if fill_n > 0:
